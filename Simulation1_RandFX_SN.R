...
#Pt. 2b: Random-Effects Simulation, Shot Noise Processes
...
genSN<-function(pars, eventFreq=.1, eventType="level", iv=c(1,0)){
  ...
  # shocks<-rbinom(pars[[1]],1,eventFreq)*sample(c(-1, 1), pars[[1]], replace=T)#
...
library(doSNOW)
  ...
  foreach(i=1:nIter,
    ...
    y<-data.frame("obs"=matrix(0,parSet$N,1), tim=1:parSet$N)
    dataGen<-readRDS("SSM_model.RDS")
    dataGen<-mxModel(dataGen, mxData(y, type="raw"))
    dataGen<-omxSetParameters(dataGen, labels=names(coef(dataGen)), values=unlist(c(parSet[4:5], parSet$dynErr, parSet$measErr^2, 1, 0))) 
    dat<-mxGenerateData(dataGen)$obs
  ...
  # LDE EA ------------------------------------------------------------------
    LDE.EA<-omxSetParameters(LDE.EA, "d2x1ts1", values=0,free=F,lbound=NA)
    LDE.EA$compute<-omxDefaultComputePlan()
    LDE.EA$compute$steps$GD<-mxComputeNelderMead(maxIter = 500000)
    
    phi<- -0.152344    +0.004885*D    +0.002200*length(dat) #NEW: VARIANCE AND BIAS (QUICK CALIBRATION): Obj value 0.317456
    AR<-D
    LDEPDA.fit<-PDA(LDE.EA, tEmbedded, thres.sd=phi,AR.order=AR, max.iter=25, plot=F)
  ...
  # SSM ---------------------------------------------------------------------
   ...
   # Output ------------------------------------------------------------------
  out<-unlist(c(parSet, "LDE"=coef(LDE)[1:2], "LDE.EA"=coef(LDEPDA.fit$fit)[1:2], "SSM"=coef(ssm)[1:2],
  ...
  write.table(t(out), sep=",", file="outputDiffProc_Rand.csv",append=ifelse(i==1, continue, T), col.names=ifelse(i==1, !continue, F), row.names=F)
  }
